"""
# to open mysql server you need to type:
#mysql -u root -p in terminal
# to look up settings on mysql go to apple icon, system settings,
and type mysql in the search bar
# pip install mysql-connector-python to install mysql connector

show databases; # to see all databases
create database school; # create a database named school
show tables from school; # to see all tables in school
use school; # use the school database
create table students( # create a table named students
id int auto_increment primary key,
# id column with auto increment taking int data
name varchar(255)
# name column taking varchar eg. variable characters / text with a max
# length of 255 characters
); # closes the table creation
select * from students;
# shows all columns in the students table
insert into students (id,name,course,mobile)
# inserts the above named rows into the students table
values("Emma Penis", "math", "0416398578");
# inserts the above values across all 4 rows in the table
exit; # exits the mysql server
...
class DatabaseConnection:
    ...
    # above information used to access the database
        self.host = host
        self.user = user
